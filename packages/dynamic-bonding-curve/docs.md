...
- [Pool Functions](#pool-functions)
    ...
    - [createConfigAndPoolAndBuy](#createConfigAndPoolAndBuy)
...
- The partner keypair is required to sign.
- The poolCreator keypair is required to sign.
- The baseMint keypair is required to sign.
- The baseMint token type must be the same as the config key's token type.
- The quoteMint must be the same as the config key's quoteMint.

---

### createConfigAndPoolAndBuy

Creates a configuration key, a token pool and creator first buy in a single transaction.

#### Function

```typescript
async createConfigAndPoolAndBuy(createConfigAndPoolAndBuyParam: CreateConfigAndPoolAndBuyParam): Promise<Transaction>
```

#### Parameters

```typescript
interface CreateConfigAndPoolAndBuyParam {
    payer: PublicKey // The wallet paying for the transaction
    config: PublicKey // The config account address (generated by the partner)
    feeClaimer: PublicKey // The wallet that will be able to claim the fee
    leftoverReceiver: PublicKey // The wallet that will receive the bonding curve leftover
...
A transaction that requires signatures from the partner keypair, creator keypair, the baseMint keypair, and the config keypair before being submitted to the network.

#### Example

```typescript
const transaction = await client.pool.createConfigAndPoolAndBuy({
    payer: new PublicKey('boss1234567890abcdefghijklmnopqrstuvwxyz'),
    config: new PublicKey('1234567890abcdefghijklmnopqrstuvwxyz'),
    feeClaimer: new PublicKey('boss1234567890abcdefghijklmnopqrstuvwxyz'),
    leftoverReceiver: new PublicKey('boss1234567890abcdefghijklmnopqrstuvwxyz'),
    ...
    swapBuyParam: {
        ...
        quoteMintTokenAccount: quoteMintAta,
